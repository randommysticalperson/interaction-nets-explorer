    ...
    content: [
      ...
      },
      {
        ...
        steps: [
          ...
          { tex: '\\phi - \\mathrm{id} \\text{ invertible} \\Leftrightarrow \\exists!\\, x^* = (\\phi - \\mathrm{id})^{-1}(-v)', label: 'Kovács Theorem 1 (unique fixed point)' },
          { tex: 'm = pr \\cap qs', label: 'Ruler construction: fixed point as intersection (Kovács Theorem 2)' },
        ],
      },
      {
        type: 'quote' as const,
        text: 'An affine transformation F = τᵥ ∘ φ has a unique fixed point if and only if φ − id is an isomorphism of ℝⁿ. Moreover, the unique fixed point is (φ − id)⁻¹(−v).',
        attribution: 'Kovács, Z. — "On the fixed points of an affine transformation: an elementary view" (2006)',
      },
      {
        type: 'prose' as const,
        text: "The Kovács paper (Teaching Mathematics and Computer Science, 2006) gives an elementary geometric construction for finding fixed points of plane affine transformations. Given a parallelogram (a, b, c, d) and its image (a\u2019, b\u2019, c\u2019, d\u2019) under F, the fixed point is the intersection m = pr \u2229 qs, where p = ab \u2229 a\u2019b\u2019, r = dc \u2229 d\u2019c\u2019, q = bc \u2229 b\u2019c\u2019, s = ad \u2229 a\u2019d\u2019. This ruler construction is the geometric analogue of the interaction net normal form: the fixed point is where all the lines of force (active pairs) converge and annihilate.",
      },
      {
        type: 'table' as const,
        headers: ['Affine Geometry (Kovács 2006)', 'Interaction Net'],
        rows: [
          { cells: ["Parallelogram (a,b,c,d) and image (a\u2019,b\u2019,c\u2019,d\u2019)", 'Net before and after one rewrite step'], tryTerm: null },
          { cells: ["Intersection p = ab \u2229 a\u2019b\u2019", 'Active pair: principal ports connected'], tryTerm: null },
          { cells: ['Fixed point m = pr ∩ qs', 'Normal form: no active pairs remain'], tryTerm: null },
          { cells: ['No fixed point: pr ∥ qs', 'Diverging term: Ω combinator'], tryTerm: '(\\x. x x)(\\x. x x)' },
          { cells: ['Pointwise-fixed line pr = qs', 'Identity: every term is already normal'], tryTerm: '\\x. x' },
          { cells: ['Equiaffine = product of two affine reflections', 'β-reduction = composition of two rewrite rules'], tryTerm: null },
        ],
        mathCols: [],
      },
